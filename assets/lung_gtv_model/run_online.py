...
def main():
    ...
    # patients_t = os.listdir(params.dict['train_data_path'])
    # patients_v = os.listdir(params.dict['val_data_path'])
    
    
    # ct_slices_pos, gt_slices_pos, pet_slices_pos, ct_slices_neg, gt_slices_neg, pet_slices_neg = utils.read_slices('positive_slices.txt', 'negative_slices.txt')
    # ct_slices_pos_v, gt_slices_pos_v, pet_slices_pos_v, ct_slices_neg_v, gt_slices_neg_v, pet_slices_neg_v = utils.read_slices('positive_slices_val.txt', 'negative_slices_val.txt')
    ...
    for iteration in range(0, params.dict['num_steps'] + 1):
        ...
        # patient = random.choice(patients_t)
        # patient_path = os.path.join(params.dict['train_data_path'], patient)
        # patient_path = os.path.join(params.dict['data_path'], patient + '/GT_All')
        # ct, gt_lung, gt_gtv = utils.load_nifti_set(patient_path)
        # ct_batch, gt_batch = load_batch(ct, gt_lung, gt_gtv, params)
        
        # ct_batch, gt_batch = sample_batch(patient_path, params)
        ...
        if iteration % params.dict['val_eval_step'] == 0:
            # patient_v = random.choice(patients_v)
            
            # patient_path_v = os.path.join(params.dict['val_data_path'], patient_v)
            # patient_path_v = os.path.join(params.dict['data_path'], patient_v + '/GT_All')
            # ct_v, gt_lung_v, gt_gtv_v = utils.load_nifti_set(patient_path_v)
            # ct_batch_val, gt_batch_val = load_batch(ct_v, gt_lung_v, gt_gtv_v, params)

            # ct_batch_val, gt_batch_val = sample_batch(patient_path_v, params)
